...
if (isset($_GET['textarea1'])){  
        ...
        $query= "update medicalfile set notes ='$update1' where id = '$id_no'" or die(mysql_error());        
   $result =mysql_query($query) or die(mysql_error());
   }           
...
//if ($grand_total > 0){   
   $query2= "UPDATE medicalfile SET doctor='$doctor',weight='$weight',temp='$temp',b_p='$b_p',height='$height' WHERE adm_no ='$adm_no' and date='$today'";
...
<form action ="in_patients_register_doctor.php" method ="GET">
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Doctor'){
     $query = "select * FROM  medicalfile WHERE doctor LIKE '%$search%' and type='Inpatient' ORDER BY app_date desc" ;
     $SQL = "select * FROM  medicalfile WHERE doctor LIKE '%$search%' and type='Inpatient' ORDER BY app_date desc" ;
   }
   if ($search_by =='Diagnosis'){
     $query = "select * FROM  medicalfile WHERE diag1,diag2,diag3 LIKE '%$search%' and type='Inpatient' ORDER BY date desc" ;
     $SQL   = "select * FROM  medicalfile WHERE diag1,diag2,diag3 LIKE '%$search%' and type='Inpatient' ORDER BY date desc" ;
   }
   if ($search_by =='Name'){
     $query = "select * FROM  medicalfile WHERE name LIKE '%$search%' and type='Inpatient' ORDER BY name,date desc" ;
     $SQL   = "select * FROM  medicalfile WHERE name LIKE '%$search%' and type='Inpatient' ORDER BY name,date desc" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  medicalfile WHERE date = '$search' and type='Inpatient' ORDER BY name,date desc" ;
     $SQL   = "select * FROM  medicalfile WHERE date = '$search' and type='Inpatient' ORDER BY name,date desc" ;
   }
 }else{
   $query= "SELECT * FROM medicalfile where (status ='To Doctor' or status ='To Laboratory' or status ='To Ward') and diag1 ='Admitted' and type='Inpatient' ORDER BY dose7" or die(mysql_error());
   $SQL  = "SELECT * FROM medicalfile where (status ='To Doctor' or status ='To Laboratory' or status ='To Ward') and diag1 ='Admitted' and type='Inpatient' ORDER BY dose7" or die(mysql_error());
}
...
//echo"<tr><th width ='10'>ID</th><th>Click to Edit Patient's Vitals.</th><th>Age</th><th>Sex</th><th>App.Date</th><th>Temp</th><th>Status</th><th>B.P</th></tr>";
echo"<tr><th width ='10'>ID</th><th>Patient's Details.</th><th>Age</th><th>Sex</th><th>Admited</th><th>Ward/Bed</th><th>Doctor</th><th>Payer</th></tr>";
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      $bed_ward = mysql_result($result,$i,"diag2");
      ...
      echo"</td>";
      echo"<td width ='200' align ='left'><a href='in_patients_doctor_edit5.php?accounts3=$bb&accounts=$aa&ref_no=$aa2&date=$update'>$desc<img src='ico/Profile.ico' alt='statement' height='12' width='12'></a>";
      echo"</td>";            
      echo"<td width ='30'>";      
      echo"$age";
      echo"</td>";      
      echo"<td width ='20'>$rate</a>";      
