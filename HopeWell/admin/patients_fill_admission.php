...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Name'){
     $query = "select * FROM  admitfile WHERE patients_name LIKE '%$search%' ORDER BY patients_name,adm_date desc" ;
     $SQL   = "select * FROM  admitfile WHERE patients_name LIKE '%$search%' ORDER BY patients_name,adm_date desc" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  admitfile WHERE adm_date = '$search' ORDER BY patients_name,adm_date desc" ;
     $SQL   = "select * FROM  admitfile WHERE adm_date = '$search' ORDER BY patients_name,adm_date desc" ;
   }

 }else{
   $query= "SELECT * FROM admitfile ORDER BY patients_name,adm_date DESC" or die(mysql_error());
   $SQL  = "SELECT * FROM admitfile ORDER BY patients_name,adm_date ORDER BY name DESC" or die(mysql_error());
}
...
echo"<tr><th width ='10'>ID</th><th>Fill Admit. Form</th><th>Age</th><th>Sex</th><th>Adm.Date</th><th>Temp</th><th>Pay Account</th><th>Weight</th><th>B.P</th><th>Last Update</th></tr>";
...
//while ($i < $num)
      ...
      echo"</td>";
      echo"<td width ='200' align ='left'><a href='patients_edit_admit_form.php?accounts3=$bb&accounts=$aa&ref_no=$aa2'>$desc<img src='ico/Profile.ico' alt='statement' height='12' width='12'></a>";
      echo"</td>";            
      echo"<td width ='30'>";      
      echo"$age";
      echo"</td>";      
