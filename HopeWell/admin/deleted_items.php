...
<form action ="deleted_items.php" method ="GET">
<table>
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Date'){
     ...
     $query = "select * FROM  deleted_items WHERE date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
     $SQL   = "select * FROM  deleted_items WHERE date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
   }
   if ($search_by =='Patient'){
     $query = "select * FROM  deleted_items WHERE patient LIKE '%$search%' ORDER BY patient" ;
     $SQL   = "select * FROM  deleted_items WHERE patient LIKE '%$search%' ORDER BY patient" ;
   }
 }else{
   $query= "SELECT * FROM deleted_items WHERE description <>'' ORDER BY date desc" or die(mysql_error());
   $SQL  = "SELECT * FROM deleted_items WHERE description <>'' ORDER BY date desc" or die(mysql_error());
}
...
echo"<tr><th width ='100'>Date</th><th width ='300'>Patient Name </th><th>Deleted by</th><th width ='300'>Deleted Item</th><th>Reason</th></tr>";
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      echo"<td width ='15%'>";      
      if ($username=='Admin'){
echo"<ahref='list_final_invoices.php?accounts=$aa8&accounts3=$aa8&accounts9=$aa9'><font color ='red'>$type</font></a>";
      }else{
      echo"$type";
      }
      echo"</td>";      
...
//echo"<td width ='100' align ='right'><ahref=javascript:popcontact55('reprint_invoice.php?accounts=$aa8&accounts3=$aa8&accounts9=$aa9')>Print<img src='ico/Profile.ico' alt='statement' height='12' width='12'></a></td>";
