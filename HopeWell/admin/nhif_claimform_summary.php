...
<form action ="nhif_claimform_summary.php" method ="GET">
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Name'){
     $query = "select * FROM  medicalfile WHERE name LIKE '%$search%' and gl_account ='NHIF' ORDER BY name,date desc" ;
     $SQL   = "select * FROM  medicalfile WHERE name LIKE '%$search%' and gl_account ='NHIF' ORDER BY name,date desc" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  medicalfile WHERE date = '$search' and gl_account ='NHIF' ORDER BY name,date desc" ;
     $SQL   = "select * FROM  medicalfile WHERE date = '$search' and gl_account ='NHIF' ORDER BY name,date desc" ;
   }

 }else{
   $query= "SELECT * FROM medicalfile where date ='$mdate' and gl_account ='NHIF' ORDER BY name,date DESC" or die(mysql_error());
   $SQL  = "SELECT * FROM medicalfile where gl_account ='NHIF' and date ='$mdate' ORDER BY name,date ORDER BY name DESC" or die(mysql_error());
}
...
echo"<tr><th width ='10'>IP Number</th><th>Patients Name</th><th>Age</th><th>Sex</th><th>Date</th><th>Pay Account</th></tr>";
...
while ($row=mysql_fetch_array($result)) 
      ...
      //$height = mysql_result($result,$i,"last_updated");
      ...
      echo"</td>";
      echo"<td width ='350' align ='left'><a href=javascript:popcontact('print_nhifclaimform.php?accounts3=$bb&accounts=$aa&ref_no=$aa2')>$desc<img src='ico/Profile.ico' alt='statement' height='12' width='12'></a>";
      echo"</td>";            
      echo"<td width ='30'>";      
      echo"$age";
      echo"</td>";      
