...
if (isset($_GET['accounts5'])){      
   ...
   $query3 = "DELETE FROM payment_doctor_tmp WHERE id ='$record'";
...
if (isset($_GET['save_cancel'])){
   ...
   $cr_amount = 0;
   $query = "select * FROM payment_doctor_tmp WHERE description >' '" ;
   $result =mysql_query($query) or die(mysql_error());
   $id = 0;
   $nRecord = 1;
   $No = $nRecord;
   ...
   while ($i < $num)
      ...
      $mydates =date('y-m-d');
      //Go and save in the doctors file
      //---------------------------------
      $query5= "INSERT INTO doctorstrfile VALUES('$codes','$desc','$date','$ref_no','$narration','Pay','-$amount','SUP','$ref_no','admin',' ','$mydates','0','$desc','0')";
      $result5 =mysql_query($query5);

   //Now go check balances in master file
   //------------------------------------
   $query3 = "select * FROM doctorsfile WHERE account_name ='$desc'" ;
   $result3 =mysql_query($query3) or die(mysql_error());
   $num3 =mysql_numrows($result3) or die(mysql_error());
   $i3=0;
   $sup_balance = 0;
   ...
   $query2= "UPDATE doctorsfile SET os_balance ='$sup_balance' WHERE account_name ='$desc'";
...
<?php
   ...
   echo"<tr><td width ='400' align ='left'><h3><br><u>PAYMENT VOUCHER</u></h3></td><tr>";
   echo"</table>";
...
echo"<img src='ico/black.jpg' style='width:750px;height:1px;'><br>";   
   ...
   //-------Sawa -------------
   $SQL = "select * FROM payment_doctor_tmp WHERE description >' '" ;
   $hasil=mysql_query($SQL, $connect);
   $id = 0;
   $nRecord = 1;
   $No = $nRecord;
...
echo "</table></b>";
   ...
   $query3 = "DELETE FROM payment_doctor_tmp WHERE description > ' '";
   $result3 =mysql_query($query3) or die(mysql_error());


    //$query= mysql_query('DROP TABLE IF EXISTS phpgrid.sales');
    //$query= "DROP TABLE IF EXISTS hmisglob_griddemo.payment_doctor_tmp";
...
// sql to create table
$sql = "CREATE TABLE payment_doctor_tmp (
...
if (isset($_GET['add_next'])){
   ...
   $query= "CREATE TEMPORARY TABLE payment_doctor_tmp IF NOT EXISTS payment_doctor_tmp (location varchar(100) NOT NULL,
   ...
   $query9= "INSERT INTO payment_doctor_tmp  VALUES('','$location','$description','$amount','$qty','$total','$total','$myDate','')";
...
if (isset($_GET['save_cancel3']))
   ...
   $query = "select * FROM payment_doctor_tmp" ;
   $result =mysql_query($query) or die(mysql_error());
   $id = 0;
   $nRecord = 1;
   $No = $nRecord;
...
<form action ="post_payment_doctors.php" method ="GET">
<?php
...
/////////////////////////////////
echo"<table><tr><td width ='100'><b>Vourcher</b></td><td width='50' align='left'><input type='text' id ='jv_no' name='jv_no' size='10' value ='$next_jv' required></td></tr><tr><td width ='100'><b>Pay Date:</b></td><td width='50' align='left'><input type='text' id ='date' name='date' size='10' value ='$mydate' required></td></tr>";
...
echo"<table bgcolor ='red' border = '0' border color = 'black'><th width ='400' align ='left'>Account Description</th><th width ='100'></th><th width ='120'></th><th width ='120'>Amount Paid</th><th width ='150' align ='right'>Action</th></th></table>";
 $myfile =gethostname();


 $query= "SELECT * FROM payment_doctor_tmp ORDER BY id ASC" or die(mysql_error());
 $result =mysql_query($query) or die(mysql_error());
 $num =mysql_numrows($result) or die(mysql_error());
 $tot =0;
 $i = 0;
 ...
 $company_identity ='BUSTANI';
 $SQL = "select * FROM payment_doctor_tmp ORDER BY id ASC" ;
 $hasil=mysql_query($SQL, $connect);
 $id = 0;
 $nRecord = 1;
 $No = $nRecord;
...
echo"<td width ='100' align ='right'><a href='post_payment_doctors.php?accounts5=$aa8'>Del<!--img src='ico/Info.ico' alt='Smiley face' height='12' width='12'--></a></td>";
}    
echo"</tr>";
...
echo"</td><td width ='100' align ='right'></td><td width ='100' align ='right'><h3>$tot_amt</h3></td><td></td><td width ='100' align ='right'></td></font>";
