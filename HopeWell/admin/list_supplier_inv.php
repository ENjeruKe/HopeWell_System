...
if (isset($_GET['print'])){
   ...
   echo"<br><br><table><tr><td width ='350'><h4>SUPPLIERS INVOICES</h4></td><td width ='500' align ='right'><b>Print Date:.$date</b></td></tr></table>";

   $query= "SELECT * FROM supplierstrfile WHERE type ='INV' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM supplierstrfile WHERE type ='INV' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $result =mysql_query($query) or die(mysql_error());
   $num =mysql_numrows($result) or die(mysql_error());
   $tot =0;
   $i = 0;
...
echo"<hr>";
echo"<table>";
echo"<tr><th width ='100'>Date</th><th width ='300' align='left'>Company Name </th><th width='100' align ='center'>Invoice No.</th><th width ='300' align='left'>Services</th><th width = '100' aligh ='right'>Amount</th><th width ='10'></th<th width = '100' aligh ='right'>Balance</th><th width ='100'></th</tr></table>";
echo"<hr>";
$hasil=mysql_query($SQL, $connect);
$id = 0;
$nRecord = 1;
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      $rate    = mysql_result($result,$i,"service");      
...
<form action ="list_supplier_inv.php" method ="GET">
<table>
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Account'){
     $query = "select * FROM  supplierstrfile WHERE type ='INV' and account_name LIKE '%$search%' ORDER BY account_name" ;
     $SQL = "select * FROM  supplierstrfile WHERE type ='INV' and account_name LIKE '%$search%' ORDER BY account_name" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  supplierstrfile WHERE type ='INV' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
     $SQL   = "select * FROM  supplierstrfile WHERE type ='INV' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
   }
   if ($search_by =='Patient'){
     $query = "select * FROM  supplierstrfile WHERE type ='INV' and others2 LIKE '%$search%' ORDER BY others2" ;
     $SQL   = "select * FROM  supplierstrfile WHERE type ='INV' and others2 LIKE '%$search%' ORDER BY others2" ;
   }
 }else{
   $query= "SELECT * FROM supplierstrfile WHERE type ='INV' and balance <>0 ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM supplierstrfile WHERE type ='INV' and balance <>0 ORDER BY date" or die(mysql_error());
}
...
echo"<tr><th width ='100'>Date</th><th width ='300'>Account Name </th><th>Invoice No.</th><th width ='300'>Services</th><th>Amount</th><th>Print</th</tr>";
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      $rate    = " ";   
      $type       = mysql_result($result,$i,"doc_no");   
      $last_trans = mysql_result($result,$i,"service");  
