...
<form action ="patients_pha_changex.php" method ="GET">

<h3>Pharmacy Amendments</h3>
<?php
//$show_it ='No';
...
if (isset($_GET['save_cancel'])){ 
   ...
   $show_it ='Yes';
   if ($payer==''){
      $query= "UPDATE transact_inv_postdated set qty ='$qty',sell_price='$price',credit_rate ='$topay' where id ='$id'" or die(mysql_error());
   }else{
      $query= "UPDATE transact_inv_postdated set qty ='$qty',sell_price='$price',credit_rate ='$topay' where id ='$id'" or die(mysql_error());
   }
   ...
   if ($payer==''){
      $query= "UPDATE transact_inv_postdated set credit_rate =sell_price*qty where invoice_no ='$ref_nos' and date = '$date'";
   }else{
      $query= "UPDATE transact_inv_postdated set credit_rate =sell_price*qty where invoice_no ='$ref_nos'";
   }
   $result =mysql_query($query) or die(mysql_error()); 

}
if (isset($_GET['accounts33x'])){ 
   $price = $_GET['account3x'];
   $id     = $_GET['accountssx'];
   $name   = $_GET['accountssx'];
   $name   = $name.'_name';
...
if ($payer==''){
    $sql="DELETE FROM  transact_inv_postdated WHERE description =''";
}else{
   $sql="DELETE FROM  transact_inv_postdated WHERE description =''";
}
...
if ($payer==''){
   $sql="SELECT * FROM  transact_inv_postdated WHERE id = '$id'";
}else{
   $sql="SELECT * FROM  transact_inv_postdated WHERE id = '$id'";
}
$result = mysql_query($sql);
...
if ($payer==''){
   $query3 = "SELECT * FROM  transact_inv_postdated WHERE id = '$id'" or 
die(mysql_error());
}else{
$query3 = "SELECT * FROM  transact_inv_postdated WHERE id = '$id'" or 
die(mysql_error());
...
while($row = mysql_fetch_array($result)) {
    ...
    echo "<td align ='left'>"."<input type='date' id ='date' name='date' size='5' value ='$date'>" . "</td></tr>";
...
if ($payer==''){
//echo 'Cash'.$ref_nos;
$sql="SELECT * FROM  transact_inv_postdated WHERE invoice_no = '$ref_nos' and date ='$date'  and description <>''";
//and location='DRUGS'
}else{
//echo 'Credit'.$ref_nos;
$sql="SELECT * FROM  transact_inv_postdated WHERE invoice_no = '$ref_nos' and date ='$date' and description <>''";
