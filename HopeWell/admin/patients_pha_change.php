...
if (isset($_GET['save_cancel'])){ 
   ...
   if ($payer==''){
      $query= "UPDATE auto_transact set credit_rate =sell_price*qty where invoice_no ='$ref_nos' and date = '$date'";
   }else{
      $query= "UPDATE auto_transact_inv set credit_rate =sell_price*qty where invoice_no ='$ref_nos'";
   }
   $result =mysql_query($query) or die(mysql_error()); 
...
while($row = mysql_fetch_array($result)) {
    ...
    echo "<td align ='left'>"."<input type='text' id ='date' name='date' size='5' value ='$date' readonly>" . "</td></tr>";
