...
<?php
 ...
 if (isset($_GET['print'])){
   ...
   echo"<h4>MONTHLY LABORATORY STOCKS USAGE REPORT <img src='space.jpg' style='width:20px;height:2px;'>"."Dated from: ".$date1."<img src='space.jpg' style='width:20px;height:2px;'>"."To :".$date2;
echo"</h4>";
...
$query = "SELECT description FROM stocklab ORDER BY description"; 	 
$result = mysql_query($query) or die(mysql_error());
while ($row=mysql_fetch_array($result)) {
      $test = $row['description'];
      $SQL ="select * FROM auto_transact_lab WHERE description ='$test' and date >='$date1' AND date <= '$date2' ORDER BY date" ;   
       $hasil=mysql_query($SQL, $connect);
       $id = 0;
       $nRecord = 1;
       $No = $nRecord;
