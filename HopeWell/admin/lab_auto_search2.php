...
while($row = mysql_fetch_array($result)) {
   ...
   //----------------------------
   $sql="DELETE FROM auto_transact_lab WHERE invoice_no ='$ref_nno' and description ='$item_name'";
   $result = mysql_query($sql);
...
//echo"Refno:".$ref_no.' Refnos:'.$ref_nos;


   $tt_price = $total*$price;
      $query5= "INSERT INTO auto_transact_lab VALUES('','DRUGS','$drug','$price','$total','$tt_price','$name','$date','$adm_no','','$ref_nos','')";
$result5 =mysql_query($query5);


//Now go and update the total price
//---------------------------------
$sql2="SELECT * FROM  auto_transact_lab WHERE date ='$date' and line_no = '$adm_no'";
...
function showUser(str) { 
    ...
    //var no = document.getElementById('dd_user_input').value;   
        ...
        xmlhttp.open("GET","lab_getservices2.php?q="+str,true);
        xmlhttp.send();
    }
}
</script>
...
<?php
 ...
 echo"<table width ='100%'><tr><td width ='50%' align ='left'>";
         ...
         $cdquery="SELECT description FROM stocklab";
...
$sql="SELECT * FROM  auto_transact_lab WHERE invoice_no ='$ref_no'";
$result = mysql_query($sql);
$found ='No';
...
while($row = mysql_fetch_array($result)) {
    ...
    if ($paid >0){
       ...
       echo "<td>" ."<a href='lab_auto_search2.php?accountss=$line&accounts33=$price&account3=$id')>"."Delete"."</a>" . "</td>";
    }
    echo "</tr>";
}
echo "</table>";
