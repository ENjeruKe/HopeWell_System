...
if (isset($_GET['print'])){
   ...
   echo"<br><br><table><tr><td width ='350'><h4>SUPPLIERS PAYMENTS LIST</h4></td><td width ='500' align ='right'><b>Print Date:.$date</b></td></tr></table>";

   $query= "SELECT * FROM supplierstrfile WHERE type ='Pay' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM supplierstrfile WHERE type ='Pay' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $result =mysql_query($query) or die(mysql_error());
   $num =mysql_numrows($result) or die(mysql_error());
   $tot =0;
   $i = 0;
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Account'){
     $query = "select * FROM  supplierstrfile WHERE type ='Pay' and account_name LIKE '%$search%' ORDER BY account_name" ;
     $SQL = "select * FROM  supplierstrfile WHERE type ='Pay' and account_name LIKE '%$search%' ORDER BY account_name" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  supplierstrfile WHERE type ='Pay' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
     $SQL   = "select * FROM  supplierstrfile WHERE type ='Pay' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
   }
   if ($search_by =='Patient'){
     $query = "select * FROM  supplierstrfile WHERE type ='Pay' and others2 LIKE '%$search%' ORDER BY others2" ;
     $SQL   = "select * FROM  supplierstrfile WHERE type ='Pay' and others2 LIKE '%$search%' ORDER BY others2" ;
   }
 }else{
   $query= "SELECT * FROM supplierstrfile WHERE type ='Pay' ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM supplierstrfile WHERE type ='Pay' ORDER BY date" or die(mysql_error());
}
