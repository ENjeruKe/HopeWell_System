...
//echo "2.0";
$narration ='Stock Transfer to -'.$location2;
      $query4= "INSERT INTO st_trans VALUES(' ','$location1','$desc','$narration','-$qty_s','TRF','$date','$username','$rate','$total','$ref_no','','','','','','')";
      $result4 =mysql_query($query4);
      
//echo "2.1";


      //Now go and Add qty transfered      
      //--------------------------------
      $query2 = "select * FROM stockfile WHERE description 
='$desc' AND location ='$location2'" ;
      ...
      echo "3";
      //if this item does not exist, please add it
      //------------------------------------------
      if ($num2<=0){
          $query4= "INSERT INTO stockfile VALUES('','$location2','$desc','$price','','$qty_s','','$desc','$cost','','')";
          $result4 =mysql_query($query4);
      }else{
      $i2 =0;
      while ($i2 < $num2)
      ...
      $qty = $qtys+$qty_s;
      $query3= "UPDATE stockfile SET qty ='$qty' WHERE description ='$desc' AND location ='$location2'";
      $result3 =mysql_query($query3);
      }
      $narration ='Stock Transfer from -'.$location1;

      $query4= "INSERT INTO st_trans VALUES('','$location2','$desc','$narration','$qty_s','TRF','$date','$username','$rate','$total','$ref_no','','','','','','')";
      $result4 =mysql_query($query4);

      $i++;
 
   }
   //First go and print transfer note
   //--------------------------------
...
//  }
// echo"</select>";
echo"</td><tr><td><b>To Store </b></td><td>";



echo"<select id='patient2' name='patient2'>";        
$cdquery="SELECT description FROM st_locationf where description<>'$store_select' ORDER BY description ";
$cdresult=mysql_query($cdquery) or die ("Query to get data from firsttable failed: ".mysql_error());            
while ($cdrow=mysql_fetch_array($cdresult)) {
  $cdTitle=$cdrow["description"];
  echo "<option>$cdTitle</option>";            
