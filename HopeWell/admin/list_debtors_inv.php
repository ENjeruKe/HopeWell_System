...
if (isset($_GET['print'])){
   ...
   echo"<br><br><table><tr><td width ='350'><h4>DEBTORS INVOICES LIST</h4></td><td width ='500' align ='right'><b>Print Date:.$date</b></td></tr></table>";

   $query= "SELECT * FROM dtransf WHERE type ='INV' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM dtransf WHERE type ='INV' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $result =mysql_query($query) or die(mysql_error());
   $num =mysql_numrows($result) or die(mysql_error());
   $tot =0;
   $i = 0;
...
<form action ="list_debtors_inv.php" method ="GET">
<table>
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Account'){
     $query = "select * FROM  dtransf WHERE type ='TRF' and account_name LIKE '%$search%' ORDER BY account_name" ;
     $SQL = "select * FROM  dtransf WHERE type ='TRF' and account_name LIKE '%$search%' ORDER BY account_name" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  dtransf WHERE type ='TRF' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
     $SQL   = "select * FROM  dtransf WHERE type ='TRF' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
   }
 }else{
   $query= "SELECT * FROM dtransf WHERE type ='TRF' and balance <>0 ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM dtransf WHERE type ='TRF' and balance <>0 ORDER BY date" or die(mysql_error());
}
...
echo"<td><input type='text' name='search' size='30'>Search :</td>";
