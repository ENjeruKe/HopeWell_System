...
if (isset($_GET['add_cancels'])){
   ...
   //   $date = $sys_date;
   //}
   //$info = $qty.'Qty -'.$unit.'day x'.$freq.'x for '.$days.' days';
   
   $info = $total.'Qty ***'.$freq.'-'.$freq2.'***'.$unit;
...
if ($founds=='Yes'){
    echo"<font color ='red'>******No Editing of drugs to an old Invoice*****</font>";
}else{
    ?>
<table>
...
if (isset($_GET['account3'])){ 
   ...
   if ($delete_reason=='No'){
       echo"<form action ='auto_search2.php' method ='GET'>";
...
while($row = mysql_fetch_array($result)) {
    ...
    if ($paid >0){
       ...
       echo "<td width ='10%'>" ."<a href='auto_search2.php?accountss=$line&accounts33=$price&account3=$id&account4=$ss&account5=$vv')>"."Delete"."</a>" . "</td>";
    }
    echo "</tr>";
}
echo "</table>";
