...
if (isset($_GET['save_cancel'])){
   ...
   while ($i3 < $num3)
     ...
     $search_no =$name.'-'.$adm_no;
        if ($search_no==$patient){
           $patientxa = mysql_result($result3,$i3,"name");  
           $adm_no  = mysql_result($result3,$i3,"adm_no");  
           $ref_no  = mysql_result($result3,$i3,"ref_no");  
        }
     $i3++;
   ...
   $query = "select * FROM lab_walkin WHERE description <>''" ;
   ...
   while ($i < $num)
    ...
    //----------------------
    $query5= "INSERT INTO auto_transact VALUES('','Lab','$desc','$total','$qty','$total','$patientxa',
'$today','$adm_no','pay','$ref_no','')";
   ...
   //-----------------------------
   $query3 = "DELETE FROM lab_walkin WHERE qty > 0 and description <>''";
   $result3 =mysql_query($query3) or die(mysql_error());

   $query3 = "update medicalfile set status ='To cash office' WHERE ref_no ='$ref_no'";
   $result3 =mysql_query($query3) or die(mysql_error());
...
//if (isset($_GET['accounts6'])){ 
   ...
   $query= "CREATE TEMPORARY TABLE lab_walkin IF NOT EXISTS lab_walkin (location varchar(100) NOT NULL,
   ...
   $query9= "INSERT INTO lab_walkin  VALUES('','$location','$description','$amount','$qty','$total','$total','$myDate','')";
...
if (isset($_GET['delete'])){
   ...
   $query3 = "DELETE FROM lab_walkin WHERE qty > 0";
   $result3 =mysql_query($query3) or die(mysql_error());
}
...
<form action ="lab_walk_in.php" method ="GET">
<?php
...
echo"<table bgcolor ='red' border = '0' border color = 'black' width ='100%'><th width ='400' align ='left'>Item Description</th><th width ='100'>Price</th><th width ='120'>Qty</th><th width ='120'>Total</th><th width ='100'>Action</th></th></table>";
 ...
 $query= "SELECT * FROM lab_walkin " or die(mysql_error());
 $result =mysql_query($query) or die(mysql_error());
 $num =mysql_numrows($result) or die(mysql_error());
 $tot =0;
 $i = 0;
 ...
 while ($row=mysql_fetch_array($hasil)) 
    ...
    if ($qty > 0){
      ...
      $aa8=$row['id'];        
 if ($desc > ' '){
    echo"<td width ='100' align ='right'><a href='lab_walk_in.php?accounts5=$aa8'>Del<img src='ico/Info.ico' 
...
if (isset($_GET['add_next'])){
   ...
   $query= "INSERT INTO lab_walkin VALUES('','$location','$item_desc','$item_price','$qty','$total','$total','$myDate','')";
   $result =mysql_query($query);
