...
$SQL= $query= "SELECT * FROM auto_diagnosis where invoice_no ='$ref_nos'";
$hasil=mysql_query($SQL, $connect);

$query= "SELECT * FROM auto_diagnosis where invoice_no ='$ref_nos'";
...
while ($row=mysql_fetch_array($hasil)){ 
      $idy      = mysql_result($result,$i,"id");          
      $foundx ='Yes';
  }

if ($foundx=='No'){
   echo"<font color ='red'><h1>Please update diagnosis before you can save.....</h1></font>";
   die();
}
...
//}else{
echo $status;
   $query4= "UPDATE appointmentf SET status='$status' WHERE adm_no ='$adm_no'";
   $result4 =mysql_query($query4);
...
if (isset($_GET['search'])){
   ...
   $query= "SELECT * FROM medicalfile where (status ='To Doctor' or status ='To Laboratory') and date ='$mdate' and type='Outpatient' ORDER BY dose7" or die(mysql_error());
   $SQL  = "SELECT * FROM medicalfile where (status ='To Doctor' or status ='To Laboratory') and date ='$mdate' and type='Outpatient' ORDER BY dose7" or die(mysql_error());
   //or (status<>'Completed' and id > 3940)
   
   //$query= "SELECT * FROM medicalfile where (date ='$mdate' and type='Outpatient' and status like '%tor%') or (id >=3940 and type='Outpatient' and status like '%tor%')  ORDER BY dose7" or die(mysql_error());
   //$SQL  = "SELECT * FROM medicalfile where (date ='$mdate' and type='Outpatient' and status like '%tor%') or (id >=3940 and type='Outpatient' and status like '%tor%')  ORDER BY dose7" or die(mysql_error());
   //or (status<>'Completed' and id > 3940)
}
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      echo"</td>";
      echo"<td width ='200' align ='left'><a href='patients_doctor_edit5.php?accounts3=$bb&accounts=$aa&ref_no=$aa2&date=$update'>$desc<img src='ico/Profile.ico' alt='statement' height='12' width='12'></a>";
      echo"</td>";            
      echo"<td width ='30'>";      
      echo"$age";
      echo"</td>";      
