...
$query = "SELECT description, search_name,location FROM stockfile WHERE description LIKE '%$user_input%' and sell_price >0 and location ='PHARMACY'";

//$query = 'SELECT description, search_name,location FROM stockfile WHERE description LIKE "%'.$user_input.'%" and sell_price >0 ';
...
while($recResult = mysql_fetch_assoc($recSql)) {
  ...
  $json_arr["id"] = "#";
  $json_arr["value"] = $recResult['description'];
  $json_arr["label"] = $recResult['description'];
