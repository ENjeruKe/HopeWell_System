...
if ($search==''){
$query  = "select * FROM st_trans WHERE trans_desc ='GRT' and >='$date1' and date <= '$date2' GROUP BY ref_no" ;
}else{
$query  = "select * FROM st_trans WHERE trans_desc ='GRT' and location='$search' and date >='$date1' and date <= '$date2' ORDER BY ref_no" ;
}
...
echo"<th><h3>Stock GRTs Report</h3></th></tr>";
echo"<div><b>";
//echo"<hr>";
...
if ($search==''){
$SQL  = "select * FROM supplierstrfile WHERE trans_desc ='GRT' and date >='$date1' and date <= '$date2' GROUP BY REF_NO" ;
}else{
$SQL  = "select * FROM st_trans WHERE trans_desc ='GRT' and location='$search' and date >='$date1' and date <= '$date2' GROUP BY ref_no" ;
}
...
while ($row=mysql_fetch_array($hasil)) 
     ...
     $adm_no   = mysql_result($result,$i,"adm_no");   
      ...
      echo"<a href=javascript:popcontact505('view_grt.php?invoice=$code')><font color ='red'>$code</font></a>";
      echo"</td>";
      echo"<td width ='10%' align ='left'>";      
      echo"$update";
      echo"</td>";
...
die();
   ...
   echo"<form action ='stocks_grt_rpt.php' method ='GET'>";
