...
<form action ="list_credit_notes.php" method ="GET">
...
echo"</FORM><table width ='400'><td align ='center'></td></table>";
   ...
   while ($row=mysql_fetch_array($hasil)) 
     ...
     $address3    = mysql_result($result,$i,"address3");   
      ...
      echo"<h3><u><p align ='left'>LIST OF CREDIT NOTES</p></u></h3>";
      echo"<font size ='2'>";
      echo"<hr>";
...
echo"<hr>";
   ...
   while ($i3 < $num3)
      ...
      //---------------------------
      $today = date('y/m/d');
      $query= "SELECT * FROM htransf WHERE type <>'RC' AND amount < 0 AND date >='$date1' AND date <='$date2' ORDER BY date" or 
      die(mysql_error());
      $result =mysql_query($query) or die(mysql_error());
      $num =mysql_numrows($result) or die(mysql_error());
      $tot =0;
      $i = 0;
      $SQL = "SELECT * FROM htransf WHERE type <>'RC' AND amount < 0  AND date >='$date1' AND date <='$date2' ORDER BY date"  or die(mysql_error());
