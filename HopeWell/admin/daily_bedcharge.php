...
if (isset($_GET['billing']))
   {
     ...
     if ($supplier==$bed){
        $query0 = "select * FROM appointmentf WHERE name >='$supplier' AND name <='$bed' AND bed_no<>' '" ;
     }else{
        $query0 = "select * FROM appointmentf WHERE bed_no<>' '" ;
     }
     ...
     while ($i0 < $num0)
       {
       $id     = mysql_result($result0,$i0,"adm_no");  
       $bed_no   = mysql_result($result0,$i0,"bed_no");   
       $patients_name    = mysql_result($result0,$i0,"name");
       $account    = mysql_result($result0,$i0,"payer");
       $id_patient = $patients_name;

//echo"$id_patient";

      //Now go and admit patient the Level in stocks file
      $query3 = "select * FROM allbedsfile WHERE patients_name ='$id_patient' and bed_no ='$bed_no'" ;
      $result3 =mysql_query($query3) or die(mysql_error());
      $num3 =mysql_numrows($result3) or die(mysql_error());
      $i3=0;
      while ($i3 < $num3)
      ...
      $query3 = "select * FROM revenuef WHERE name ='BED CHARGES'" ;
      $result3 =mysql_query($query3) or die(mysql_error());
      $num3 =mysql_numrows($result3) or die(mysql_error());
      $gledger ='Bed Income';
      ...
      $query= "INSERT INTO htransf VALUES('$id','$patients_name','$date','$ref_no','$desc','GBED','$rate','IP','$gledger','DB',' ','ADMIN',' ','$mydate','1','$account')";
...
// mysql_select_db($dbname, $link) or die ("Error selecting specified database on mysql server: ".mysql_error());
 ...
 $cdquery="SELECT name FROM appointmentf WHERE bed_no<>' '";
