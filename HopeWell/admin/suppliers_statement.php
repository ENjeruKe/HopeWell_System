...
﻿<?php
 ...
 if (isset($_GET['print'])){
   ...
   echo"<h2><u>SUPPLIERS STATEMENT OF ACCOUNT</u></h2>";

   //<img src='space.jpg' style='width:20px;height:2px;'>
   //Get the account Number of this very account
   //--------------------------------------------
   $query= "SELECT * FROM suppliersfile WHERE account_name ='$search' ORDER BY account_name" or die(mysql_error());
   $result =mysql_query($query) or die(mysql_error());
   $num =mysql_numrows($result) or die(mysql_error());
   $tot =0;

   $i = 0;
   $SQL = "SELECT * FROM suppliersfile WHERE account_name ='$search' ORDER BY account_name" or die(mysql_error());
   ...
   echo"<tr><td align ='left'><b>$acc_no</b></td><td width ='500'></td><td align ='left'><b>$company</b></td></tr>";
   echo"<tr><td align ='left'><b>$acc_name</b></td><td width ='500'></td><td align ='left'><b>$address1</b></td></tr>";
   echo"<tr><td align ='left'><b>$caddress2</b></td><td width ='500'></td><td align ='left'><b>$address2</b></td></tr>";
   echo"<tr><td align ='left'><b>$caddress1</b></td><td width ='500'></td><td align ='left'><b>$address3</b></td></tr>";
   echo"</table><br>";
   $to ='  To  ';
   echo"Report Date Range From :.$date1.  $to. $date2";
   $query  = "select * FROM supplierstrfile WHERE company = '$acc_name' AND date >='$date1' AND date <= '$date2' ORDER BY date"; 
...
echo"<th width ='10%' bgcolor ='black'><font color ='white'>Type</th>";
...
if (isset($_GET['search'])){
   $search = $_GET['search'];
   $SQL = "select * FROM supplierstrfile WHERE company = '$acc_name'  AND date >='$date1' AND date <= '$date2' ORDER BY date"; 
 }else{
  $SQL= "SELECT * FROM supplierstrfile ORDER BY date" or die(mysql_error());
}
$hasil=mysql_query($SQL, $connect);
$id = 0;
$nRecord = 1;
...
<?php
   ...
   echo"<h1>Suppliers Statement of Account</h1><br><br>";
   echo"<form action ='suppliers_statement.php' method ='GET'>";
   //echo"<input type='submit' name='print'  class='button' value='Print Report'>";

   echo"<table><tr><td>Supplier A/c</td><td>";
