...
if (isset($_GET['search'])){
   // echo "1";
   $search = $_GET['search'];
   if ($acc_name=='CASH COLLECTION A/C'){
      $SQL    = "select * FROM gltransf WHERE account_name = '$acc_name' and date >='$date1' and date <='$date2' and type<>'Pay' ORDER BY date"; 
      $query  = "select * FROM gltransf WHERE account_name = '$acc_name' and date >='$date1' and date <='$date2' and type<>'Pay' ORDER BY date"; 
   }else{
...
if (isset($_GET['search'])){
   $search = $_GET['search'];
   if ($acc_name=='CASH COLLECTION A/C'){
   $SQL = "select * FROM gltransf WHERE account_name = '$acc_name' AND date >='$date1' AND date <= '$date2' and type <>'Pay' 
ORDER BY date"; 
   }else{
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      $contact = mysql_result($result,$i,"branch");  
      $street  = mysql_result($result,$i,"type");  
       $spe  = mysql_result($result,$i,"inputby");  
      $pay_mode= mysql_result($result,$i,"type");  
      $total   = mysql_result($result,$i,"amount");  
     $code2 = $bik;
...
if ($desc ='pay'){
     echo"<td width ='40%' align ='left'>";      
      echo strtolower("$service").' '.strtolower("$spe");
      echo"</td>";
...
<?php
   ...
   echo"<form action ='gl_transactions.php' method ='GET'>";
