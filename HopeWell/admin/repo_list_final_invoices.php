...
if (isset($_GET['print'])){
   ...
   $query= "SELECT * FROM hdnotef WHERE inv_amount <>0 AND date >='$from_date' AND date <= '$to_date' ORDER BY date DESC" or die(mysql_error());
   $SQL  = "SELECT * FROM hdnotef WHERE inv_amount <>0 AND date >='$from_date' AND date <= '$to_date' ORDER BY date DESC" or die(mysql_error());
   $result =mysql_query($query) or die(mysql_error());
   $num =mysql_numrows($result) or die(mysql_error());
   $tot =0;
   $i = 0;
...
<form action ="repo_list_final_invoices.php" method ="GET">
<table>
...
if (isset($_GET['search'])){
   ...
   $query= "SELECT * FROM hdnotef WHERE inv_amount <>0 ORDER BY date DESC" or die(mysql_error());
   $SQL  = "SELECT * FROM hdnotef WHERE inv_amount <>0 ORDER BY date DESC" or die(mysql_error());
}
...
while ($row=mysql_fetch_array($hasil)) 
      ...
      echo"<td width ='60'>";      
      if ($username=='admin'){
echo"<a href='repo_list_final_invoices.php?accounts=$aa8&accounts3=$aa8&accounts9=$aa9'><font color ='red'>$type</font></a>";
      }else{
      echo"$type";
      }
      echo"</td>";      
      ...
      $aa8=$row['invoice_no'];
      $aa9= date('y-m-d');
      if ($last_trans =='NHIF UNIVERSAL'){
echo"<td width ='100' align ='right'><a href=javascript:popcontact55('reprint_nhif_invoice.php?accounts=$aa8&accounts3=$aa8&accounts9=$aa9')>Print<img src='ico/Profile.ico' alt='statement' height='12' width='12'></a></td>";
      }else{          
