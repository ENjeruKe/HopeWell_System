...
page(_($help_context = "Doctors"), false, false, "", $js);
...
function supplier_settings(&$supplier_id)
{
	...
	text_row(_("Doctors Name:"), 'supp_name', null, 42, 40);
	text_row(_("Doctors Short Name:"), 'supp_ref', null, 30, 30);
	...
	{
		label_row(_("Doctor's Currency:"), $_POST['curr_code']);
		hidden('curr_code', $_POST['curr_code']);
	} 
	else 
	{
		currencies_list_row(_("Doctors's Currency:"), 'curr_code', null);
	}
	tax_groups_list_row(_("Tax Group:"), 'tax_group_id', null);
	text_row(_("Our Customer No:"), 'supp_account_no', null, 42, 40);

	table_section_title(_("Consultants"));
	...
	gl_all_accounts_list_row(_("Consultants Account:"), 'payable_account', $_POST['payable_account']);
	gl_all_accounts_list_row(_("Consultants Account Link:"), 'purchase_account', $_POST['purchase_account'],
		false, false, _("Use Item Inventory/COGS Account"));
	gl_all_accounts_list_row(_("Doctors Discount Account:"), 'payment_discount_account', $_POST['payment_discount_account']);
	if (!$supplier_id) {
	...
	if ($supplier_id)
		record_status_list_row(_("Doctors status:"), 'inactive');
	end_outer_table(1);
	...
	if ($supplier_id) 
	{
		submit_center_first('submit', _("Update Doctor"), 
		...
		submit_center_last('delete', _("Delete Doctor"), 
		  _('Delete doctor data if have been never used'), true);
	}
	else 
	{
		submit_center('submit', _("Add New Doctor Details"), true, '', 'default');
	}
	div_end();
}
...
if (isset($_POST['submit'])) 
{
	...
	{
		$input_error = 1;
		display_error(_("The doctors name must be entered."));
		set_focus('supp_name');
	}
	...
	{
		$input_error = 1;
		display_error(_("The doctor short name must be entered."));
		set_focus('supp_ref');
	}
	...
	if ($input_error !=1 )
	{
		...
		else 
		{
			...
			display_notification(_("A new doctor has been added."));
...
elseif (isset($_POST['delete']) && $_POST['delete'] != "") 
{
	...
	{
		$cancel_delete = 1;
		display_error(_("Cannot delete this doctor because there are transactions that refer to this doctor."));
	...
	else 
	{
		...
		{
			$cancel_delete = 1;
			display_error(_("Cannot delete the doctor record because invoices have been created against this doctor."));
		}
	...
	if ($cancel_delete == 0) 
	{
		...
		$supplier_id = '';
		$Ajax->activate('_page_body');
		display_notification("#" . $_POST['supplier_id'] . " " . _("Doctor has been deleted."));
...
if (db_has_suppliers()) 
{
	...
	supplier_list_cells(_("Select a supplier: "), 'supplier_id', null,
		  _('New doctor'), true, check_value('show_inactive'));
