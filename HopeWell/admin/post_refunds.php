...
if (isset($_GET['save_cancel'])){
   ...
   while ($i < $num)
      ...
      //--------------------
   $query9= "INSERT INTO receiptf VALUES ('','EXPENSE','$debit_account','$narrate','1','Refunds','$date','$username','-$total','-$total','$ref_no','-$total','','','$timew')";
      $result9 =mysql_query($query9);


//Reduce from pay_data cash
//-------------------------
   $query5a= "INSERT INTO pay_datacash VALUES ('','EXPENSE','$debit_account','$narrate','1','Refunds','$date','$username','-$total','-$total','$ref_no','-$total','','','$timew')";
   $result5a =mysql_query($query5a);

    


      $narrate =$rate.': :'.$narration;
      //narrations;   
      //Go and dabit gltransf A/c
      //-------------------------
      $query5= "INSERT INTO gltransf 
VALUES('','$date','$debit_account','$ref_no','Pay','$narrate','$total','admin','INCOME')";
      ...
      //-------------------------
      $query5= "INSERT INTO gltransf 
      VALUES('','$date','$credit_account','$ref_no','Pay','$narrate','-$total','$username','CONTROL')";
      $result5 =mysql_query($query5);
...
function showUser(str) {    
    ...
    if (str == "") {
        ...
        xmlhttp.open("GET","getrefundaccount.php?q="+str,true);
        xmlhttp.send();
    }
}
</script>
...
<form action ="post_refunds.php" method ="GET">
<?php
 ...
 while ($row=mysql_fetch_array($hasil)) 
      ...
      if ($desc > ' '){
        ...
        ='$desc'></td>";
         ...
         $cdquery="SELECT account_name FROM glaccountsf WHERE account_name like '%REFUND%'";
         $cdresult=mysql_query($cdquery) or die ("Query to get data from firsttable failed: ".mysql_error());
         $query3 = "select * FROM glaccountsf WHERE account_name like '%REFUND%'";
         $result3 =mysql_query($query3) or die(mysql_error());
         $num3 =mysql_numrows($result3) or die(mysql_error());
         $i3=0;
