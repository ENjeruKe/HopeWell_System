...
if (isset($_GET['print'])){
   ...
   echo"<br><br><table><tr><td width ='350'><h4>DEBTORS CREDIT NOTES</h4></td><td width ='500' align ='right'><b>Print Date:.$date</b></td></tr></table>";

   $query= "SELECT * FROM dtransf WHERE type ='CR' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM dtransf WHERE type ='CR' AND date >='$from_date' AND date <= '$to_date' ORDER BY date" or die(mysql_error());
   $result =mysql_query($query) or die(mysql_error());
   $num =mysql_numrows($result) or die(mysql_error());
   $tot =0;
   $i = 0;
...
echo"<table>";   
      ...
      echo"<td width ='300'>";
      echo"</td>";      
      echo"<td width ='100'><h4>Total Amount:</h4>";      
      echo"</td>";      
      echo"<td width ='300' align ='right'>";
      echo"</td>";      
      //echo"<td width ='50' align ='right'></td>";      
      echo"<td width ='40' align='right'><h4>$tot2</h4></td>";    
      echo"</table>";
//echo"<hr>";
die();
...
<form action ="list_debtors_cr.php" method ="GET">
<table>
...
if (isset($_GET['search'])){
   ...
   if ($search_by =='Account'){
     $query = "select * FROM  dtransf WHERE type ='CR' and account_name LIKE '%$search%' ORDER BY acc_no" ;
     $SQL = "select * FROM  dtransf WHERE type ='CR' and account_name LIKE '%$search%' ORDER BY acc_no" ;
   }
   if ($search_by =='Date'){
     $query = "select * FROM  dtransf WHERE type ='CR' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
     $SQL   = "select * FROM  dtransf WHERE type ='CR' and date >= '$from_date' AND date <='$to_date' ORDER BY date" ;
   }
   if ($search_by =='Patient'){
     $query = "select * FROM  dtransf WHERE type ='CR' and others2 LIKE '%$search%' ORDER BY others2" ;
     $SQL   = "select * FROM  dtransf WHERE type ='CR' and others2 LIKE '%$search%' ORDER BY others2" ;
   }
 }else{
   $query= "SELECT * FROM dtransf WHERE type ='CR' and balance <>0 ORDER BY date" or die(mysql_error());
   $SQL  = "SELECT * FROM dtransf WHERE type ='CR' and balance <>0 ORDER BY date" or die(mysql_error());
}
