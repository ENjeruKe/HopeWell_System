...
if (isset($_GET['Cash'])){   
   $ref_nos = $_GET['Cash'];
   $query3= "UPDATE medicalfile SET status ='To Cash Office' WHERE ref_no ='$ref_nos'";
   $result3 =mysql_query($query3);
}
...
if (isset($_GET['search'])){
   ...
   $query= "SELECT * FROM medicalfile where status ='To Pharmacy' and (date ='$mdate' or time<>'') ORDER BY dose7" or die(mysql_error());
   $SQL  = "SELECT * FROM medicalfile where status ='To Pharmacy' and (date ='$mdate' or time<>'') ORDER BY dose7" or die(mysql_error());
}
