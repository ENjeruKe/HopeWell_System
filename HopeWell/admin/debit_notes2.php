  ...
  //Asign ECT value if selected
   ...
   if ($supplier == "DOCTORS FEE"){
      //if ($mamount5 == 0){
         ...
         '$amount','INV','','admin',' ','$date','','$desc','$amount')";
...
echo "15";
   ...
   if ($gledger=='ECT-INCOME' or $gledger == 'ECT-CHARGES'){
      ...
      if ($mamount5 > 0){
         $query5= "INSERT INTO doctorstrfile VALUES('','$codes','$doc_account','$date','$ref_no','$patient_ref','INV',
         '$mamount5','INV','','admin',' ','$date','','$descs','$mamount5')";
         $result5 =mysql_query($query5);
      }
...
('','$adm_no','$patient,'$date','$ref_no','E.C.T','DB','$amount','$chargable','$gledger','DB'
,'','ADMIN','$mydate','1','$pay_company','')";

   }else{
      $query= "INSERT INTO htransf 
      VALUES('','$adm_no','$patient','$date','$ref_no','$supplier','DB','$amount','DB','$gledger','IP','','ADMIN','','$mydate','1','$pay_company')";
...
echo "17";
   ...
   if ($gledger =='ECT-INCOME' OR $gledger =='ECT-CHARGES'){
      $desc ='E.C.T - '.$patient_ref;
      $query= "INSERT INTO gltransf VALUES(' ','$date','$debt_control','$ref_no','DB','$desc','$amount','ADMIN','$company')";
      $result =mysql_query($query);
      $query = "select * FROM glaccountsf WHERE account_name='$debt_control'" ;
      $result =mysql_query($query) or die(mysql_error());
      $num =mysql_numrows($result) or die(mysql_error());
